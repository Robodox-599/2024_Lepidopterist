...
public class RobotContainer {
  ...
  /* Controllers */  
  private final CommandXboxController driver = new CommandXboxController(ControllerConstants.xboxDriveID);
  ...
  public RobotContainer() {
    ...
    // m_Intake.setDefaultCommand(new command_ManualIntakeWrist(m_Intake, () -> -operator.getRightY()));
...
/*  -------------------
    ...
    driver.rightBumper().whileTrue(Commands.sequence(
      ...
      m_Indexer.sourceIndexer()),
      new InstantCommand(() -> m_Shooter.stopFeeder(), m_Shooter)));
    driver.rightBumper().onFalse(m_Shooter.stowAndStopShooter().andThen(m_Indexer.sourceIndexer()).withTimeout(0.5).andThen(m_Shooter.stowShooter()));
...
/*  ---------------------
    ...
    //                                               new command_AutoSpeaker(m_DriveTrain, m_Shooter, () -> false));
    ...
    // driver.leftTrigger().negate().and(driver.rightTrigger()).whileTrue(shootSequence(m_Indexer, m_Shooter));
    ...
    //subwoofer setpoint
    operator.leftBumper().and(operator.rightBumper().negate()).whileTrue(new command_ToWristAndSpeed(m_Shooter, 
                                      () -> ShooterConstants.WristSepoints.testSpeakerWrist,
                                      () -> ShooterConstants.FlywheelSetpoints.SpeakerSpeed));
    operator.leftBumper().and(operator.rightBumper()).whileTrue(m_Shooter.waitUntilReady().andThen(
                                                    shootSequence(m_Indexer, m_Shooter).withTimeout(1.0)));
    operator.leftBumper().negate().and(operator.rightBumper().negate()).onTrue(m_Shooter.stowShooter());
    ...
    operator.back().whileTrue(m_Shooter.forwardsFeederStartEnd().alongWith(m_Indexer.runIndexerStartEnd()));

  }

  public void addAutos(){
    NamedCommands.registerCommand("Extend Intake", Commands.parallel(m_Intake.autonIntake(), m_Indexer.runIndexerUntilBeamBreak()).withTimeout(2.5));
    ...
    Commands.sequence(
      ...
      Commands.parallel(m_Shooter.forwardsFeederStartEnd().withTimeout(2.0), 
    ...
    // NamedCommands.registerCommand("Shoot Line", new command_AutoSpeaker(m_DriveTrain, m_Shooter, () -> true).until(m_Shooter.isReadyToShoot())
    // .andThen(shootSequence(m_Indexer, m_Shooter)).withTimeout(3.0).andThen(m_Shooter.stowShooter()));


    NamedCommands.registerCommand("Shoot Line", Commands.sequence(
      new command_ToWristAndSpeed(m_Shooter, () -> ShooterConstants.WristSepoints.testSpeakerWrist,
                                          () -> ShooterConstants.FlywheelSetpoints.SpeakerSpeed),
      new WaitUntilCommand(m_Shooter.isReadyToShoot()), 
      Commands.parallel(m_Shooter.forwardsFeederStartEnd().withTimeout(2.0), 
  ...
  public Command shootSequence(subsystem_Indexer indexer, subsystem_Shooter shooter){
    return m_Shooter.setFlywheelSpeedCommand(() -> FlywheelSetpoints.testFlywheelSetpoint)
