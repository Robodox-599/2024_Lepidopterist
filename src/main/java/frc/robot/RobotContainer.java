...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    m_driverController.y().onTrue(Commands.sequence( m_flywheels.toggle_feeder_Command(() -> false), command_cGroup_shooter.stow(m_flywheels, m_wrist, m_LED, m_drivetrain)));
// y is stow
      //x speaker shoot ready button  
      m_driverController.x().onTrue(command_cGroup_shooter.armSpeakerCommand(m_flywheels, m_wrist, m_LED, m_drivetrain));
      //make a change to the to amp writst commmand and add a paramet for stow
      m_driverController.leftBumper().whileTrue(m_flywheels.toggle_feeder_Command(()->true));
  }
