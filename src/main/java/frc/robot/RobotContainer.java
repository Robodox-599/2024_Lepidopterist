...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    m_Shooter.setDefaultCommand(m_Shooter.manualWrist(() -> -operator.getLeftY(), 
  ...
  private void configureBindings() {
    ...
    driver.leftTrigger().and(driver.rightTrigger()).whileTrue(
                                    ...
                                    m_LED.setAnimationCommand(LEDAnimation.Shooting, () -> 1.0))));
    ...
    driver.leftTrigger().onFalse(Commands.parallel(new command_ToWristAndSpeed(m_Shooter, 
                                () -> ShooterConstants.WristSepoints.minShootAngle, 
                                () -> ShooterConstants.FlywheelSetpoints.StowSpeed),
                                m_LED.setLEDStateCommand(null)));
    ...
    // Amp Score
    operator.rightBumper().whileTrue(m_Intake.ampScoreCommand().andThen(
                                    m_LED.setAnimationCommand(LEDAnimation.Amping, 
                                                              () -> 1.0)));
    operator.rightBumper().onFalse(m_Intake.stowCommand().andThen(
                                    m_LED.setLEDStateCommand(null)));
  ...
  //   m_Chooser.addOption("Shoot Auto", Autos.runAutoPath(m_DriveTrain, "Shoot Auto"));

  //   NamedCommands.registerCommand("Run Intake", runIntakeSequence(m_Intake, m_Indexer, m_LED));
  ...
  //   NamedCommands.registerCommand("spkrLeft", Commands.parallel(new InstantCommand(() -> m_Indexer.runIndexer(), m_Indexer), 
  //                                                                   m_Shooter.spkrLeft()).andThen(new command_ToWristAndSpeed(m_Shooter, 
  //                                                                                       () -> ShooterConstants.WristSepoints.minShootAngle, 
  //                                                                                       () -> ShooterConstants.FlywheelSetpoints.StowSpeed),
  //                                                                                       new InstantCommand(() -> m_Indexer.stopIndexer(), m_Indexer)));
  //   NamedCommands.registerCommand("spkrMid", Commands.parallel(new InstantCommand(() -> m_Indexer.runIndexer(), m_Indexer), 
  //                                                                   m_Shooter.spkrMid()).andThen(new command_ToWristAndSpeed(m_Shooter, 
  //                                                                                       () -> ShooterConstants.WristSepoints.minShootAngle, 
  //                                                                                       () -> ShooterConstants.FlywheelSetpoints.StowSpeed),
  //                                                                                       new InstantCommand(() -> m_Indexer.stopIndexer(), m_Indexer)));
  //   NamedCommands.registerCommand("spkrRight", Commands.parallel(new InstantCommand(() -> m_Indexer.runIndexer(), m_Indexer), 
  //                                                                   m_Shooter.spkrRight()).andThen(new command_ToWristAndSpeed(m_Shooter, 
  //                                                                                       () -> ShooterConstants.WristSepoints.minShootAngle, 
  //                                                                                       () -> ShooterConstants.FlywheelSetpoints.StowSpeed),
  //                                                                                       new InstantCommand(() -> m_Indexer.stopIndexer(), m_Indexer)));
  ...
  public static Command runIntakeSequence(subsystem_Intake intake, subsystem_Indexer indexer, subsystem_LED led){
    return Commands.parallel(
      ...
      .andThen(led.setAnimationCommand(LEDAnimation.Intook, () -> 1.0))
      ; 
  }
