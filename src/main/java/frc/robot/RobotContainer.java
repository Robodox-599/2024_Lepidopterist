...
import frc.robot.commands.command_extend;
...
// y is stow
      ...
      m_driverController.x().whileTrue(command_cGroup_shooter.sourceIntake(m_flywheels, m_wrist, m_LED));
      m_driverController.x().onFalse(command_cGroup_shooter.stow(m_flywheels, m_wrist, m_LED, m_drivetrain));

      m_driverController.y().whileTrue(command_cGroup_shooter.scoreAmp(m_flywheels, m_wrist, m_LED));
      m_driverController.y().onFalse(command_cGroup_shooter.stow(m_flywheels, m_wrist, m_LED, m_drivetrain));

  // m_driverController.a().onTrue(new command_extend(m_rod,()->60));
  m_driverController.leftBumper().onTrue(new command_extend(m_Indexer,()->Constants.Setpoints.stow));
  m_driverController.leftStick().onTrue(new command_extend(m_Indexer, ()->15)); // stop
  m_driverController.rightBumper().onTrue(new command_extend(m_Indexer, ()->20));// reset bb


      // for indexer _ is for reset, _ is to stop, _ is for forward, _ is for back. 
  }
