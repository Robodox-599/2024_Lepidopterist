...
public class IntakeCommands extends Command {
  ...
  public static Command intakeDeployAndIntake(IntakeWrist wrist, Rollers rollers, Indexer indexer){
    return Commands.sequence(extendCommand(wrist), runIndexerUntilBeamBreak(indexer), runIntakeuntilBeamBreak(rollers));
  }

  public static Command intakeStartEnd(IntakeWrist wrist, Rollers rollers, Indexer indexer, double time){
    return new StartEndCommand(()-> intakeDeployAndIntake(wrist, rollers, indexer).andThen(new WaitUntilCommand(time)), ()-> stowCommand(wrist));
  }
}
