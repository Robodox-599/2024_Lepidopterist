...
public class command_DriveTeleop extends Command {
  ...
  public void initialize() {
    if (RobotConstants.robotColor == null){
      SmartDashboard.putString("blliance", "Eror");
    } else if (RobotConstants.robotColor == Alliance.Blue){
      SmartDashboard.putString("blliance", "Blue");
    } else if (RobotConstants.robotColor == Alliance.Red){
      SmartDashboard.putString("blliance", "Red");
    } else {
      SmartDashboard.putString("blliance", "Mega ERROR");
    }
    if(RobotConstants.robotColor != null){
  ...
  public void execute() {
    ...
    if (m_DriveTrain.getDriveState() == DRIVE_STATE.SHOOTER_PREP){
      ...
      if (RobotConstants.robotColor == Alliance.Red){
        ...
        double tempDangle = 180.0 + Math.atan2(deltaY, deltaX) * UnitConstants.RAD_TO_DEG;
