...
public class command_DriveTeleop extends Command {
  ...
  private BooleanSupplier m_IsExtended;
  private BooleanSupplier m_IsRetracted;
  ...
  public command_DriveTeleop(subsystem_DriveTrain driveTrain,
                            ...
                            BooleanSupplier isExtended,
                            BooleanSupplier isRetracted,
  ...
  public void execute() {
    ...
    if(Math.abs(m_xSpeed.getAsDouble()) > ControllerConstants.deadband){
      transformedXSpeed = m_DriveTrain.setLinearThrottle(2.0 * m_xSpeed.getAsDouble());
    }

    if(Math.abs(m_ySpeed.getAsDouble()) > ControllerConstants.deadband){
      transformedYSpeed = m_DriveTrain.setLinearThrottle(2.0 * m_ySpeed.getAsDouble());
    }
    ...
    m_DriveTrain.setClimbSetpoint(m_IsExtended.getAsBoolean(), m_IsRetracted.getAsBoolean());
  }
