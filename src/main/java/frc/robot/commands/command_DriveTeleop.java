...
public class command_DriveTeleop extends Command {
  ...
  private Translation2d m_speakerCenter;
  ...
  public void execute() {
    ...
    if(Math.abs(m_xSpeed.getAsDouble()) > OperatorConstants.deadband){
      transformedXSpeed = m_DriveTrain.setThrottle(2.0 * m_xSpeed.getAsDouble());
    }

    if(Math.abs(m_ySpeed.getAsDouble()) > OperatorConstants.deadband){
      transformedYSpeed = m_DriveTrain.setThrottle(2.0 * m_ySpeed.getAsDouble());
    }

    if(Math.abs(m_zRot.getAsDouble()) > OperatorConstants.deadband){
