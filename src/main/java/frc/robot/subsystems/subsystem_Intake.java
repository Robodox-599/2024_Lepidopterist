...
public class subsystem_Intake extends SubsystemBase {
  ...
  public subsystem_Intake() {
    m_IntakeMotor = new TalonFX(IntakeConstants.RollerMotorConstants.intakeRollersMotorID, RobotConstants.CANBus);
    m_IntakeMotor.setNeutralMode(NeutralModeValue.Brake);
    
    m_WristMotor = new TalonFX(IntakeConstants.WristMotorConstants.wristMotorID, RobotConstants.CANBus);
  ...
  public void setDesiredWristPos(double passedInPosition){
    ...
    MotionMagicVoltage m_request = new MotionMagicVoltage(m_DesiredWristPos).withSlot(m_WristSlot).withFeedForward(WristMotorConstants.kWristFeedForward);
    m_WristMotor.setControl(m_request.withPosition(m_DesiredWristPos));
  }

  public boolean isWristAtDesiredPosition(){
    SmartDashboard.putNumber("diffsintake", Math.abs(m_WristMotor.getPosition().getValueAsDouble() - m_DesiredIntakePos));
  ...
  public Command runIntakeuntilBeamBreak(){
    return Commands.sequence(runIntakeCommand(() -> IntakeConstants.kSpeakerIntakeSpeed), 
  ...
  public Command moveWristCommand(DoubleSupplier passedInPosition){
    return Commands.sequence(new InstantCommand(() -> setDesiredWristPos(passedInPosition.getAsDouble()), this), 
                new WaitUntilCommand(() -> {return isWristAtDesiredPosition();}));
  }
  ...
  public Command runIntakeBackCMD(){
    return new StartEndCommand(() -> m_IntakeMotor.set(IntakeConstants.kIntakeBackfeedSpeed), ()->stopIntake(), this);
    // return new StartEndCommand(() -> runIntake(true, false, false), () -> stopIntake(), this);
  }

  public Command runIntakeFwdCMD(){
    return new StartEndCommand(() -> m_IntakeMotor.set(IntakeConstants.kSpeakerIntakeSpeed), ()->stopIntake(), this);
    // return runIntakeCommand(() -> IntakeConstants.kSpeakerIntakeSpeed).repeatedly().finallyDo(() -> stopIntake());
  }

  public Command autonExtend(){
    // return runIntakeCommand(() -> IntakeConstants.kAutoIntakeSpeed).repeatedly().finallyDo(() -> stopIntake());
    return new StartEndCommand(() -> m_IntakeMotor.set(IntakeConstants.kAutoIntakeSpeed), ()->stopIntake(), this);
  }
  ...
  public void periodic() {
    ...
    SmartDashboard.putBoolean("beambreak1", m_beamBreakTimer.get() >= IndexerConstants.beamBreakDebounce);
  }
