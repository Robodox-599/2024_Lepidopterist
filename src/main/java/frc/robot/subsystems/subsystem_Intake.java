...
public class subsystem_Intake extends SubsystemBase {
  ...
  public void runIntake(boolean isBackward){
    ...
    double sign = isBackward ? -1.0 : 1.0;
    ...
    m_IntakeMotor.set(sign * IntakeConstants.kIntakeSpeed);//IntakeConstants.kIntakeSpeed  }
  }
  ...
  public boolean isWristAtDesiredPosition(double passedInPosition){
    return (Math.abs(m_IntakeMotor.getPosition().getValueAsDouble() - passedInPosition) < IntakeConstants.kWristTolerance);
  }
  ...
  public Command moveWristCommand(DoubleSupplier passedInPosition){
    return Commands.sequence(new InstantCommand(() -> setDesiredWristPos(passedInPosition.getAsDouble()), this), 
                new WaitUntilCommand(() -> {return isWristAtDesiredPosition(m_DesiredIntakePos);}));
  }

  public void stow(){
    stopIntake(); 
    setDesiredWristPos(IntakeConstants.kWristRetractVal);
  }
  ...
  public Command ampIntakeCommand(){
    return Commands.sequence(
      ...
      new WaitUntilCommand(() -> m_IntakeMotor.getSupplyCurrent().getValueAsDouble() >= IntakeConstants.IntakeNoNoteCurrent),
  ...
  public void ampTake(){
    setDesiredWristPos(IntakeConstants.kWristAmpVal);
  ...
  public void periodic() {
    ...
    SmartDashboard.putBoolean("At intake Desired Wrist Position", isWristAtDesiredPosition(m_DesiredWristPos));
