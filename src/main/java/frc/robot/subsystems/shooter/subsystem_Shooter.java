...
public class subsystem_Shooter extends SubsystemBase {
  ...
  private double[] rad_list = {
    1.07, 1.307, 1.57, 1.809
  }; // TODO: update values here after collecting empirically
  // 1.8, 2.0, 2.22, 2.75, 4.0
  private double[] angle_list = {51.8, 45.3, 41.5, 38.5};
  ...
  private TrapezoidProfile.State m_previousState =
      new TrapezoidProfile.State(WristSepoints.minShootAngle, 0);
  ...
  public subsystem_Shooter() {
    speakerCenter3D =
        RobotConstants.robotColor == Alliance.Red
    ...
    m_RightFlywheel =
        new TalonFX(RightFlywheelMotorConstants.rightFlywheelID, RobotConstants.CANBus);
    ...
    m_WristFeedforward =
        new ArmFeedforward(
            WristMotorConstants.wristKS,
            WristMotorConstants.wristKV,
            WristMotorConstants.wristKG,
            // * Math.cos(MotorRotToShootAngle(m_Wrist.getPosition().getValueAsDouble()) *
            // UnitConstants.DEG_TO_RAD),
  ...
  public void setFlywheelSpeed(double velocity) {
    ...
    // m_DesiredFlywheelSpeed = velocity / (SurfaceSpeed.shooter_transfer *
    // SurfaceSpeed.shooter_rad*SurfaceSpeed.shooter_GR * 2.0 * Math.PI); //gets rpms
  }
  ...
  public boolean flywheelsAtDesiredSpeed() {
    return ((Math.abs(
                m_RightFlywheel.getVelocity().getValueAsDouble()
  ...
  public void setDesiredShootAngle(double angle) {
    ...
    if (angle <= ShooterConstants.WristSepoints.maxShootAngle
        && angle >= ShooterConstants.WristSepoints.minShootAngle) {
  ...
  public boolean isWristAtDesiredPosition() {
    return Math.abs(
            shootAngletoMotorRot(m_DesiredShootAngle) - m_Wrist.getPosition().getValueAsDouble())
        <= ShooterConstants.wristTolerance;
  }
  ...
  // public InstantCommand manualWrist(DoubleSupplier wristJoystickInput, BooleanSupplier
  // max_speed){
  //   double m_newDesired;
  //   if (Math.abs(wristJoystickInput.getAsDouble())>ControllerConstants.deadband){
  //    m_newDesired = m_DesiredShootAngle + wristJoystickInput.getAsDouble() *
  // ShooterConstants.max_manual_ratio;
  ...
  public void runFeeder() {
    // double feeder_rpm = SurfaceSpeed.feeder_max_surface * 60.0 /
    // (SurfaceSpeed.feeder_rad*SurfaceSpeed.feeder_GR * 2.0 * Math.PI);
  ...
  public Command waitUntilReady() {
    ...
    // SmartDashboard.putBoolean("isWithinPos",
    // speakerCenter2D.getDistance(drivePose.getTranslation())
  ...
  public BooleanSupplier isReadyToShoot() {
    return () ->
        (m_flywheelTimer.get() >= ShooterConstants.shootTime
  ...
  public Command autoSourceShooter() {
    return Commands.sequence(
        ...
        new WaitUntilCommand(
            () -> (m_FlySpikeTimer.get() >= ShooterConstants.flywheelSpikeDebounce)),
        stowShooterSource());
  }
  ...
  public double MotorRotToShootAngle(double motorRot) {
    ...
    double theta =
        ((180.0 + WristMathConstants.driverArmOffset)
                - (motorRot / WristMathConstants.wristGearRatio) * 360.0)
            * UnitConstants.DEG_TO_RAD;
    ...
    double beta =
        Math.acos((Math.pow(d_14, 2) + Math.pow(l1, 2) - Math.pow(r1, 2)) / (2.0 * d_14 * l1));
    double d_45 = Math.hypot(P4[0], P4[1]);
    double gamma =
        Math.acos(
            (Math.pow(d_14, 2) + Math.pow(P1[0], 2) - Math.pow(d_45, 2)) / (2.0 * d_14 * P1[0]));
    double shotAngle =
        ((beta - gamma) * UnitConstants.RAD_TO_DEG) - WristMathConstants.shooterAngleToPivotAngle;
    if (theta_deg < 180.0) {
      shotAngle =
          ((beta + gamma) * UnitConstants.RAD_TO_DEG) - WristMathConstants.shooterAngleToPivotAngle;
    }
  ...
  public double shootAngletoMotorRot(double shootAngle) {
    ...
    double netAngle =
        (shootAngle + WristMathConstants.shooterAngleToPivotAngle) * UnitConstants.DEG_TO_RAD;
    ...
    double motorPosRotations =
        linkageAngleFromStart * WristMathConstants.wristGearRatio * UnitConstants.DEG_TO_ROT;
  ...
  public void periodic() {
    ...
    SmartDashboard.putNumber("desired shoot angle", m_DesiredShootAngle);
    if (m_LeftFlywheel.getStatorCurrent().getValueAsDouble()
            <= ShooterConstants.flywheelSourceCurrent
        && m_RightFlywheel.getStatorCurrent().getValueAsDouble()
            <= ShooterConstants.flywheelSourceCurrent) {
      m_FlySpikeTimer.restart();
    }
    SmartDashboard.putBoolean(
        "flySpikeLeft",
        m_LeftFlywheel.getStatorCurrent().getValueAsDouble()
            > ShooterConstants.flywheelSourceCurrent);
    SmartDashboard.putBoolean(
        "flySpikeRight",
        m_RightFlywheel.getStatorCurrent().getValueAsDouble()
            > ShooterConstants.flywheelSourceCurrent);
    SmartDashboard.putBoolean(
        "flySpike", m_FlySpikeTimer.get() >= ShooterConstants.flywheelSpikeDebounce);
    ...
    SmartDashboard.putNumber(
        "left flywheel current", m_LeftFlywheel.getStatorCurrent().getValueAsDouble());
    SmartDashboard.putNumber(
        "right flywheel current", m_RightFlywheel.getStatorCurrent().getValueAsDouble());
    ...
    SmartDashboard.putNumber(
        "desired Shooter motor Angle", shootAngletoMotorRot(m_DesiredShootAngle));
    SmartDashboard.putNumber(
        "Shooter Wrist Position", MotorRotToShootAngle(m_Wrist.getPosition().getValueAsDouble()));
    ...
    double wFF =
        m_WristFeedforward.calculate(
            m_DesiredShootAngle * UnitConstants.DEG_TO_RAD,
            Units.degreesToRadians(MotorRotToShootAngle(m_Wrist.getVelocity().getValueAsDouble())));
    // double wFF =
    // ShooterConstants.wristFeedForward.get(shootAngletoMotorRot(m_DesiredShootAngle));
    ...
    if (m_DesiredShootAngle == 0.0) {
      ...
      if (m_DesiredShootAngle != ShooterConstants.WristSepoints.minShootAngle) {
        m_Wrist.setControl(
            new MotionMagicVoltage((shootAngletoMotorRot(m_DesiredShootAngle)))
                .withSlot(0)
                .withFeedForward(wFF));
      } else {
        m_Wrist.stopMotor();
    ...
    if (max_speed_wheel) {
      ...
      // if (m_LeftFlywheel.getVelocity().getValueAsDouble() -
      // ShooterConstants.FlywheelSetpoints.FlywheelCoastMargin< m_DesiredFlywheelSpeed){
    ...
    if (max_speed_wheel) {
      ...
      if (m_DesiredFlywheelSpeed >= ShooterConstants.flywheelDifferential) {
        // if (m_RightFlywheel.getVelocity().getValueAsDouble() -
        // ShooterConstants.FlywheelSetpoints.FlywheelCoastMargin <
        // m_DesiredFlywheelSpeed-ShooterConstants.flywheelDifferential){

        m_RightFlywheel.setControl(
            new VelocityVoltage(m_DesiredFlywheelSpeed - ShooterConstants.flywheelDifferential));
        // }
