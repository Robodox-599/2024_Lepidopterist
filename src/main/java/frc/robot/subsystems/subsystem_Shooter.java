...
public class subsystem_Shooter extends SubsystemBase {
  ...
  public Command spkrLeft(){
    return Commands.sequence(
      // TODO: add angle for flush against subwoofer
      Commands.parallel(
      new InstantCommand(() -> setDesiredShootAngle(AutoConstants.AutoWristSetpoints[0]), this),
      new InstantCommand(() -> setFlywheelSpeed(AutoConstants.AutoFlywheelSetpoints[0]), this)),
      new WaitUntilCommand(() -> (isWristAtDesiredPosition() && flywheelsAtDesiredSpeed()))
      .andThen(forwardsFeederStartEnd().withTimeout(1)),
      Commands.parallel(
      new InstantCommand(() -> setDesiredShootAngle(ShooterConstants.WristSepoints.minShootAngle), this),
      new InstantCommand(() -> setFlywheelSpeed(ShooterConstants.FlywheelSetpoints.StowSpeed), this)));
 }
  
  public Command spkrMid(){
    return Commands.sequence(
      // TODO: add angle for flush against subwoofer
      Commands.parallel(
      new InstantCommand(() -> setDesiredShootAngle(AutoConstants.AutoWristSetpoints[6]), this),
      new InstantCommand(() -> setFlywheelSpeed(AutoConstants.AutoFlywheelSetpoints[6]), this)),
      new WaitUntilCommand(() -> (isWristAtDesiredPosition() && flywheelsAtDesiredSpeed()))
      .andThen(forwardsFeederStartEnd().withTimeout(1)),
      Commands.parallel(
      new InstantCommand(() -> setDesiredShootAngle(ShooterConstants.WristSepoints.minShootAngle), this),
  ...
  public Command spkrRight(){
    return Commands.sequence(
      // TODO: add angle for flush against subwoofer
      Commands.parallel(
      new InstantCommand(() -> setDesiredShootAngle(AutoConstants.AutoWristSetpoints[7]), this),
      new InstantCommand(() -> setFlywheelSpeed(AutoConstants.AutoFlywheelSetpoints[7]), this)),
      new WaitUntilCommand(() -> (isWristAtDesiredPosition() && flywheelsAtDesiredSpeed()))
      .andThen(forwardsFeederStartEnd().withTimeout(1)),
      Commands.parallel(
      new InstantCommand(() -> setDesiredShootAngle(ShooterConstants.WristSepoints.minShootAngle), this),
