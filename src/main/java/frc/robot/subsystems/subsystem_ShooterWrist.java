...
public class subsystem_ShooterWrist extends SubsystemBase {
  ...
  public subsystem_ShooterWrist() {
    m_wrist = new TalonFX(Constants.ShooterWristConstants.MotorConstants.wristID,
        Constants.ShooterWristConstants.Canbus);
    m_wristConfig = new TalonFXConfiguration();
    // m_wristEncoder = new Encoder(Constants.ShooterWristConstants.MotorConstants.DIO_pin1,
    //     Constants.ShooterWristConstants.MotorConstants.DIO_pin2);
    ...
    // profile = new TrapezoidProfile(new
    // TrapezoidProfile.Constraints(Constants.ShooterWristConstants.MotorConstants.max_velocity,
    // Constants.ShooterWristConstants.MotorConstants.max_accel));

    motor_speed = 0;
    m_wrist.getConfigurator().apply(m_wristConfig);
    // m_wristConfig = new TalonFXConfiguration();
    // m_RelativeEncoder = new
    // Encoder(Constants.ShooterWristConstants.MotorConstants.DIO_pin1,
    // Constants.ShooterWristConstants.MotorConstants.DIO_pin2);
    // m_wristConfig.Slot0.kP =
    // Constants.ShooterWristConstants.PIDConstants.wristkP;
    // m_wristConfig.Slot0.kI =
    // Constants.ShooterWristConstants.PIDConstants.wristkI;
    // m_wristConfig.Slot0.kD =
    // Constants.ShooterWristConstants.PIDConstants.wristkD;
    // m_RelativeEncoder = m_wrist.getEncoder();
    // m_PidController = m_wrist.getPIDController();
    // absEncoderDuty = new
    // DutyCycleEncoder(Constants.ShooterWristConstants.absEncoderChannel);
    // m_wrist.getConfigurator().apply(m_wristConfig);
    // absEncoderDuty.set
  ...
  public void set_wrist(double angle) {
    // if (angle >= ShooterWristConstants.minWrist && angle <=
    // ShooterWristConstants.maxWrist)
  ...
  public boolean isWristAtDesiredPosition() {
    return Math.abs(m_wrist.getPosition().getValueAsDouble()
        - m_desired_wrist) <= ShooterWristConstants.wristError;

        // return Math.abs(m_wristEncoder.getRaw() * Constants.ShooterWristConstants.MotorConstants.ticks_to_rotations
        // - m_desired_wrist) <= ShooterWristConstants.wristError;    
    // return Math.abs((m_wrist.getPosition().getValueAsDouble() - m_desired_wrist)
    // )
  ...
  public void periodic() {
    ...
    SmartDashboard.putNumber("encoder", m_wrist.getPosition().getValueAsDouble());
    ...
    // TrapezoidProfile.State m_goal = new TrapezoidProfile.State(m_desired_wrist,
    // 0);
    ...
    if (!isWristAtDesiredPosition()) {
      ...
      // motor_speed =
      // m_PidController.calculate(m_wrist.getPosition().getValueAsDouble(),
      // m_desired_wrist);
