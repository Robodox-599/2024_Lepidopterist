...
public class subsystem_ShooterWrist extends SubsystemBase {
  ...
  public subsystem_ShooterWrist() {
    m_wrist = new CANSparkMax(Constants.ShooterWristConstants.MotorConstants.wristID,
  ...
  public boolean isWithinMargin(double margin) {
    // return
    // Math.abs(m_wristEncoder.getRaw()*Constants.ShooterWristConstants.MotorConstants.ticks_to_rotations
    // - m_desired_wrist_angle)
    // <= ShooterWristConstants.wristError;
    return Math.abs((absEncoderDuty.getAbsolutePosition() - startPos)
        * ShooterWristConstants.MotorConstants.rotations_to_angle
        - m_desired_wrist_angle) <= margin;
  }

  @Override
  public void periodic() {
    // This method will be called once per scheduler run
    SmartDashboard.putBoolean("wrist ok", isWithinMargin(ShooterWristConstants.wristError));
    // if (//encoder shit)
    // if
    // (!(Math.abs(m_wristEncoder.getRaw()*Constants.ShooterWristConstants.MotorConstants.ticks_to_rotations
    // - m_desired_wrist_angle)>Constants.ShooterWristConstants.wristError)){
    ...
    if (!isWithinMargin(ShooterWristConstants.wristError)) {
