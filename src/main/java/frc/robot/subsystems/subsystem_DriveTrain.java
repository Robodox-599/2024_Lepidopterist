...
public class subsystem_DriveTrain extends SubsystemBase {
  ...
  public subsystem_DriveTrain() {
    ...
    m_PoseEstimator =
        new SwerveDrivePoseEstimator(
            ...
            m_ModulePositions,
            new Pose2d(3.0, 5.0, new Rotation2d()),
            vec1,
            vec2);
  ...
  public void swerveDrive(
    ...
    setRobotSpeeds(
        ChassisSpeeds.fromFieldRelativeSpeeds(
            xSpeedMetersPerSecond, ySpeedMetersPerSecond, zRotRadiansPerSecond, getPoseYaw()));

    SwerveModuleState[] moduleStates =
        SwerveConstants.kinematics.toSwerveModuleStates(getRobotSpeeds());
  ...
  public void setModuleStates(SwerveModuleState[] desiredStates) {
    SwerveDriveKinematics.desaturateWheelSpeeds(
        desiredStates, AutoConstants.MaxSpeedMetersPerSecond);
  ...
  public double setLinearThrottle(double input) {
    return m_LinearThrottle == Throttle.LINEAR
        ? input
        : Math.signum(input) * (1.01 * Math.pow(input, 2) - 0.0202 * input + 0.0101);
  }

  public double setAngularThrottle(double input) {
    return m_AngularThrottle == Throttle.LINEAR
        ? input
        : Math.signum(input) * (1.01 * Math.pow(input, 2) - 0.0202 * input + 0.0101);
  }
  ...
  //     case CENTER:
  //       resetPose = isBlue ? FieldConstants.blueSubwooferCenter :
  // FieldConstants.redSubwooferCenter;
  //     case RIGHT:
  ...
  public Rotation2d getYaw() {
    double yaw =
        SwerveConstants.invertGyro
            ? 360.0 - m_Gyro.getYaw().getValueAsDouble()
  ...
  public double transformZRot(double oldZRot) {
    ...
    if (oldZRot != 0
        || m_state == DRIVE_STATE.DRIVER_CONTROL) { // note deadband already accounted for earlier
  ...
  public boolean readyToShoot() {
    return Math.abs(
            m_PoseEstimator.getEstimatedPosition().getRotation().getDegrees()
                - m_AutoOrientPID.getSetpoint())
        < SwerveConstants.readyToShootThreshold;
  }
  ...
  public void driveRobotRelative(ChassisSpeeds speeds) {
    ...
    //                                                     speeds, getPoseYaw()));
    SwerveModuleState[] moduleStates =
        SwerveConstants.kinematics.toSwerveModuleStates(ChassisSpeeds.discretize(speeds, 0.02));
  ...
  public void periodic() {
    ...
    SmartDashboard.putNumber(
        "Distance Error", getPose().getTranslation().getDistance(m_FinalPose.getTranslation()));
